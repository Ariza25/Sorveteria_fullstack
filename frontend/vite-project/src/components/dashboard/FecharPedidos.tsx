...
const FecharPedidos = () => {
  ...
  const getOrdersDone = async () => {
    try {
      ...
      const ordersDone = response.data.filter(
        (order: Order) => order.finishOrder === false && order.deliveryStatusDone === true 
      );
  ...
  const updateStatusToDeliveryDone = useCallback(
    async (order: { id: string }) => {
      ...
      try {
        const response = await api.put(`http://localhost:3333/v1/api/order?id=${id}`, { deliveryStatusDonet: false, finishOrder: true });
  ...
  return (
    ...
    <ToastContainer/>
      <main className="flex flex-col py-10 px-10 w-full">
        ...
        {ordersDone
          .filter((order) => order.finishOrder === false)
          .map((order) => (
            <div
              key={order.id}
